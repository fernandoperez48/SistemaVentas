        ...
        .then(data => {
            ...
            data.forEach((envios, index) => {
                ...
                document.body.insertAdjacentHTML('beforeend', modalUpdate);
                 ...
                 // Script para actualizar el envío
                 document.getElementById(`btn_update${id_envio}`).addEventListener('click', function() {
                    var direccion = document.getElementById(`direccion${id_envio}`).value;
                    var estado = document.getElementById(`estado${id_envio}`).value;

                    if (direccion === '') {
                        document.getElementById(`lbl_direccion${id_envio}`).style.display = 'block';
                        document.getElementById(`direccion${id_envio}`).focus();
                    } else if (estado === '') {
                        document.getElementById(`lbl_estado${id_envio}`).style.display = 'block';
                        document.getElementById(`estado${id_envio}`).focus();
                    } else {
                        var url = "../app/controllers/envios/update.php";
                        fetch(url + `?id_envio=${id_envio}&direccion=${direccion}&estado=${estado}`)
                            .then(response => response.text())
                            .then(datos => {
                                document.getElementById(`respuesta_update${id_envio}`).innerHTML = datos;
                            })
