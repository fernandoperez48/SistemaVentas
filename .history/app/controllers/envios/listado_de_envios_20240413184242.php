...
$sql_envios ="SELECT e.IdVenta,COALESCE(emp.nombre, p.nombre) AS nombre_cliente, e.Direccion, e.estado,c.id_persona,v.total_pagado,v.nro_venta,v.fyh_creacion
FROM tb_envios as e
inner Join tb_clientes as c on c.id_cliente = e.IdCliente
INNER JOIN tb_empresas AS emp ON c.id_empresa = emp.id_empresa
INNER JOIN tb_personas AS p ON c.id_persona = p.id_persona 
