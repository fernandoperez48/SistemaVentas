...
class PDF extends FPDF
{
    ...
    function TablaProductos($productos, $total_cantidad, $total_precio_unitario, $total_precio)
    {
        ...
        if (isset($_POST['id_venta'])) {
            ...
            $this->Cell(20, 7, 'Cod. Prod.', 1);
            $this->Cell(60, 7, 'Producto/Servicios', 1);
            $this->Cell(15, 7, 'Cantidad', 1, '', 'C');
            $this->Cell(15, 7, 'U. Medida',  1, '', 'C');
            $this->Cell(16, 7, 'Precio Unit.',  1, '', 'C');
            $this->Cell(12, 7, '% Bonif',  1, '', 'C');
            $this->Cell(18, 7, 'Subtotal',  1, '', 'C');

            $this->MultiCell(12,  3.5, 'Alicuota' . "\n" . 'IVA', 1, 'C', '');
            $this->SetXY(178, 90);
            $this->Cell(22, 7, 'Subtotal c/IVA',  1, '', 'C');
            $this->Ln();

            // Filas de productos
            $this->SetFont('Arial', '', 9);
            // $productos = [
            //     ['p-00001', 'Boina con guardapampa', '3', 'unidades', '76205', 'o,oo', '228615,00', '21%', '276624,15'],
            //     ['p-00001', 'Boina con guardapampa', '3', 'unidades', '76205', 'o,oo', '228615,00', '21%', '276624,15'],
    ...
    function Totales()
    {
        $this->SetFont('Arial', 'B', 30);
        $this->SetXY(10, 190);
        $this->Cell(190, 60, 's', '1', '1', 'C');
        $this->SetFont('Arial', '', 9);
        $this->SetY(190);
        $this->Cell(0, 5, 'Importe Neto Gravado: $ 879439,00', 0, 1, 'R');
        $this->Cell(0, 5, 'IVA 21%: $ 184682,19', 0, 1, 'R');
        $this->Cell(0, 5, 'Importe Total: $ 1064121,19', 0, 1, 'R');
        // $this->Ln(10);
        // Detalle CAE
...
$pdf->FacturaDetalle();
$pdf->TablaProductos($productos, $total_cantidad, $total_precio_unitario, $total_precio);
